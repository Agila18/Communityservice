...
class _HealthNotebookScreenState extends State<HealthNotebookScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
        context.read<VoiceService>().speak("உங்கள் மருத்துவ குறிப்பேடு. புதிய ரிப்போர்ட்டை சேர்க்க கேமராவை அழுத்தவும்.");
    });
  }
  ...
  Future<void> _captureAndUploadRecord() async {
    ...
    context.read<VoiceService>().speak("படிக்கிறேன், ஒரு நிமிடம் காத்திருங்கள்..."); // Reading, please wait
    ...
    try {
      ...
      setState(() => _isUploading = false);
      context.read<VoiceService>().speak("மன்னிக்கவும், ரிப்போர்ட்டை சேமிக்க முடியவில்லை.");
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text("மருத்துவ குறிப்பேடு", style: AppTextStyles.headingMedium), // Health Notebook
          ...
          ? Center(
              child: Column(
                ...
                children: [
                   ...
                   Text("ரிப்போர்ட்டை படிக்கிறேன்...", style: AppTextStyles.headingMedium, textAlign: TextAlign.center),
                   const SizedBox(height: 8),
                   Text("தயவுசெய்து காத்திருக்கவும்", style: AppTextStyles.bodyLarge.copyWith(color: AppColors.textSecondary)),
                ],
              ),
            )
          : _isLoading 
      ...
      floatingActionButton: SizedBox(
        ...
        child: FloatingActionButton.extended(
          ...
          label: Text("புகைப்படம் எடு", style: AppTextStyles.headingMedium.copyWith(color: Colors.white)), // Take Photo
        ),
      ),
     ...
     return Center(
       child: Padding(
         ...
         child: Column(
           mainAxisAlignment: MainAxisAlignment.center,
           children: [
             Icon(Icons.library_books_rounded, size: 96, color: AppColors.primary.withValues(alpha: 0.2)),
             const SizedBox(height: 24),
             Text("எந்த தகவலும் இல்லை", style: AppTextStyles.headingLarge), // No data
  ...
  Widget _buildGalleryList() {
    return ListView.builder(
      padding: const EdgeInsets.all(16.0).copyWith(bottom: 100), // Clearance for giant FAB
                ...
                Row(
                  children: [
                    Container(
                      ...
                      child: Icon(isPrescription ? Icons.medical_information_rounded : Icons.science_rounded, color: AppColors.primary),
                    ),
                    ...
                    Expanded(
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                           Text(record['title'] ?? "மருத்துவ அறிக்கை", style: AppTextStyles.headingMedium),
                    ...
                    IconButton(
                       icon: const Icon(Icons.volume_up_rounded, color: AppColors.riskGreen, size: 32),
                       onPressed: () => _playRecordAudio(record['ai_explanation']),
                    )
                  ],
                ),
                const SizedBox(height: 16),
